...
export default function Navbar() {
  ...
  const linkClass = `text-sm font-medium tracking-wide transition-colors cursor-pointer ${
    isScrolled ? 'text-white/80 hover:text-[#dbf226]' : 'text-[#005b52] hover:text-[#04221f]/70'
  }`

  const btnClass = `rounded-full px-5 py-2 text-sm font-bold transition-colors ${
    isScrolled
  ...
  return (
    <nav
      className={`fixed z-50 flex items-center justify-between transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] ${
        isScrolled
          ? 'top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl rounded-full bg-[#04221f] text-white shadow-2xl px-8 py-3'
